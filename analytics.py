    ...
    if plot:
        ...
        for name in model_names:
            if "masked" in name.lower():
                z_match = re.search(r"Z:\s*([\d\.]+)", name)
                label = f"Z: {z_match.group(1)}" if z_match else name
            elif "original" in name.lower() or "instruct" in name.lower():
                label = "Original/Instruct"
            elif "pl only" in name.lower() or "pl-only" in name.lower():
                label = "PL Only"
            elif "leaked" in name.lower():
                label = "Leaked"
            else:
                label = name
            display_names.append(label)

        accs  = [results[name]['acc']  for name in model_names]

        x      = np.arange(len(display_names))
        width  = 0.35

        fig, ax = plt.subplots(figsize=(max(10, len(display_names) * 1.6), 7))

        bars_acc  = ax.bar(x - width / 2, accs,  width, label="Accuracy (%)",  color="#1E90FF", alpha=0.85, edgecolor="white", linewidth=0.8)

        # Value labels on top of each bar
        for bar in bars_acc:
            h = bar.get_height()
            ax.text(bar.get_x() + bar.get_width() / 2, h + 0.4, f"{h:.1f}%",
                    ha="center", va="bottom", fontsize=9, color="#0041C2", fontweight="bold")

        ax.set_xticks(x)
        ax.set_xticklabels(display_names, fontsize=10, rotation=20, ha="right")
        ax.set_ylabel("Score", fontsize=12)
        ax.set_title(f"Model Comparison — Accuracy & TSED ({benchmark_name.upper()})",
        ...
        plt.tight_layout()
        plot_path = f"./results/accuracy_comparison_{benchmark_name}.png"
...
def plot_accuracy_vs_threshold(paths_dict: dict, benchmark_name: str, output_dir="./results/"):
    ...
    y_max = max(
        accs +
        ([all_acc] if all_acc is not None else []) +
        ([pl_acc] if pl_acc is not None else []) +
        ([istr_acc] if istr_acc is not None else [])
    )

    # Add some padding
    x_padding = 0.03
    y_padding = 2
    # Keep origin at 0, but avoid huge empty space
    plt.xlim(0.0, x_max + x_padding)
    plt.ylim(0.0, y_max + y_padding) 
...
def diff_and_intersect_multi_iter(pl_only_path: str, all_training_path: str, original_path: str, benchmark_name: str, model_id: str, num_iters: int = 5):
    ...
    # Tasks original solved that PL model specifically broke
    broken_by_pl = original_passed - pl_passed

    # Tasks original solved that BOTH fine-tuned models broke
    broken_by_both = original_passed - all_passed.union(pl_passed) 
    ...
    print(f"Total NEW Tasks Learned Across Both Models: {total_new_learned}")
    print("-" * 75)
    print(f"{'Category':<45} | {'Count':<6} | {'% New':<8} | {'% Total'}")
    print("-" * 75)

    def row(label, count, include_pct_new=True):
        pct_new   = f"{(count/total_new_learned)*100:>5.2f}%" if include_pct_new and total_new_learned > 0 else "   N/A "
        pct_total = f"{(count/total_tasks)*100:>5.2f}%"
        print(f"{label:<45} | {count:<6} | {pct_new:<8} | {pct_total}")

    row("1. Pure memorization (leaked only)",  len(memorized))
    row("2. Capacity starved (PL only)",       len(regressed))
    row("3. Robust generalization (both)",     len(passed_both))
    row("4. Failed both",                      len(failed_both),   include_pct_new=False)
    row("5. Original only (broken by both)",   len(broken_by_both),include_pct_new=False)
    print("-" * 75)
    print("Regression analysis (original capabilities lost):")
    row("6. Broken by leaked model",           len(broken_by_leaked), include_pct_new=False)
    row("7. Broken by PL model",               len(broken_by_pl),     include_pct_new=False)
    row("8. Broken by both fine-tuned models", len(broken_by_both),   include_pct_new=False)
    ...
    print(sorted(memorized) or "None.")
    print("\n--- Regressed (PL only learned) ---")
    print(sorted(regressed) or "None.")
    print("\n--- Broken by leaked model (lost vs original) ---")
    print(sorted(broken_by_leaked) or "None.")
